...
$(document).ready(function () {
   tbData = $("#periodeTable").DataTable({
      ...
      fixedHeader: !0,
      ajax: {
         url: $("#periodeTable").attr("data-url") || LOCALE_URL + "/dashboard/periode-all",
         data: function (req) {
...
$(document)
   ...
   .on("click", "button.action-status", function (e) {
      ...
      Swal.fire({
         title: "Ubah Status Data " + name + "?",
         text: "Pilih status data yang diinginkan dibawah ini:",
         icon: "question",
         input: "select",
         inputOptions: {
         ...
         $.ajax({
            ...
            success: function (res) {
               if (res.status == 200) {
                  ...
                  $("form#form-data [name=id_semester]").val(data.id_semester);
                  $("form#form-data [name=nama_periode]").val(data.nama_periode);
                  $("form#form-data [name=tanggal_pelaksanaan]").val(data.tanggal_pelaksanaan);
                  $("form#form-data [name=waktu_mulai]").val(data.waktu_mulai);
                  $("form#form-data [name=waktu_selesai]").val(data.waktu_selesai);
                  $("form#form-data [name=tanggal_gladi]").val(data.tanggal_gladi);
                  $("form#form-data [name=waktu_gladi]").val(data.waktu_gladi);
                  $("form#form-data [name=lokasi]").val(data.lokasi);
                  $("form#form-data [name=link_google_maps]").val(data.link_google_maps);
                  $("form#form-data [name=link_survey]").val(data.link_survey);
                  $("form#form-data [name=tema_wisuda]").val(data.tema_wisuda);
                  $("form#form-data [name=makna_wisuda]").val(data.makna_wisuda);
                  dataModal.show(modalElement);
               } else {
                  Swal.fire({
                     title: errorMessage,
...
$("[name=icon_indikator]").keyup(function () {
   var icon = $(this).val();
